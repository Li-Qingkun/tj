<template>
  <div class="page-container">
      ...
      <el-form :inline="true"
        ...
        <el-form-item>
          <el-input v-model="filters.name"
                    placeholder="排放口名称"></el-input>
        </el-form-item>
        <el-form-item>
          <kt-button icon="fa fa-search"
                     label="查询"
                     perms="ep:outfall:view"
                     type="primary"
                     @click="findPage(null)" />
        </el-form-item>
        <el-form-item>
    ...
    <!--表格内容栏-->
    <kt-table permsEdit="ep:outfall:update"
              permsDelete="ep:outfall:delete"
              :data="pageResult"
              :columns="columns"
              @findPage="findPage"
    ...
    <el-dialog :title="operation?'新增':'编辑'"
      ...
      <el-form :model="dataForm"
        ...
        <el-form-item label="简称"
                      prop="abbreviation">
          <el-input v-model="dataForm.abbreviation"
                    auto-complete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="平台ID"
                      prop="subid">
          <el-input v-model="dataForm.subid"
                    auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="状态"
                      prop="produceState">
          <el-select v-model="dataForm.produceState"
                     placeholder="请选择"
                     style="width: 100%;">
            <el-option label="生产"
                       :value="1">生产</el-option>
            <el-option label="停产"
                       :value="0">停产</el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="污染物"
          ...
          <el-checkbox-group v-model="dataForm.pollutantIds">
            <el-checkbox v-for="item in pollutants"
                         :key="item.id"
                         :label="item.id"
  ...
  data() {
    return {
      ...
      dataFormRules: {
        ...
        subid: [{ required: true, message: '请输入平台ID', trigger: 'blur' }],
        produceState: [
