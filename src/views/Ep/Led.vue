<template>
  <div class="page-container">
    ...
    <!--表格内容栏-->
    <kt-table permsDetails="ep:led:view"
              permsEdit="ep:led:update"
              permsDelete="ep:led:delete"
              :data="pageResult"
              :columns="columns"
              @findPage="findPage"
    ...
    <el-dialog :title="operation?'新增':'编辑'"
      ...
      <el-form :model="dataForm"
        ...
        <el-form-item label="电子屏名称"
                      prop="name">
          <el-input v-model="dataForm.name"
                    auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="电子屏类型"
                      prop="type">
        ...
        <el-form-item label="是否语音"
                      prop="audio">
          <el-radio v-model="dataForm.audio"
                    label="1">是</el-radio>
          <el-radio v-model="dataForm.audio"
                    label="0">否</el-radio>
        </el-form-item>
        <el-form-item label="MAC"
                      prop="mac">
          <el-input v-model="dataForm.mac"
                    auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="污染物起始位置"
                      prop="pollutantIndex">
          <el-input-number v-model="dataForm.pollutantIndex"
                           :min="0"
                           :max="15"
                           style="width: 100%;"></el-input-number>
        </el-form-item>
        <el-form-item label="广告位起始位置"
                      prop="advertIndex">
          <el-input-number v-model="dataForm.advertIndex"
                           :min="0"
                           :max="15"
                           style="width: 100%;"></el-input-number>
        </el-form-item>
        <el-form-item label="广告行数"
                      prop="advertRows">
          <el-input-number v-model="dataForm.advertRows"
                           :min="0"
                           :max="15-dataForm.advertIndex"
                           style="width: 100%;"></el-input-number>
        </el-form-item>
        <el-form-item label="宽度"
                      prop="width">
          <el-input-number v-model="dataForm.width"
                           :min="0"
                           style="width: 100%;"></el-input-number>
        </el-form-item>
        <el-form-item label="高度"
                      prop="height">
          <el-input-number v-model="dataForm.height"
                           :min="0"
                           style="width: 100%;"></el-input-number>
        </el-form-item>
        <el-form-item label="排放口"
          ...
          <el-tabs v-model="tabsSelect"
                   @tab-click="handlePollutantSelect"
                   :tab-position="'top'"
                   style="height: 120px;">
            <el-tab-pane v-for="item in outfalls"
                         :name="item.id"
                         :key="item.id"
                         :label="item.name"
                         :value="item.id">
              <el-checkbox-group v-model="pollutantName">
                <el-checkbox v-for="p in pollutants"
                             @change="cbxChange(p.id)"
                             :key="p.id"
                             :label="p.name"
                             :value="p.id">{{p.name}}</el-checkbox>
              </el-checkbox-group>
            </el-tab-pane>
          </el-tabs>
        </el-form-item>
        <el-form-item label="安装日期"
                      prop="installDate">
          <el-date-picker v-model="dataForm.installDate"
                          type="date"
                          placeholder="安装日期"
    ...
    <el-dialog :title="'详情'"
      ...
      <table class="tab-details"
        ...
        <tr>
          <th>排放口</th>
          <td>
            <table v-for="o in outfallPollutants"
                   :key="o.outfallName"
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        { prop: 'signal', label: '信号强度', minWidth: 80 },
        {
          ...
          label: '心跳时间',
          minWidth: 80,
          formatter: this.heartTimeFormatter,
        },
      ],
  ...
  methods: {
    ...
    handleDetails: function (params) {
      ...
      this.$api.led.get({ id: params.row.id }).then((res) => {
        if (res.code == 200) {
          ...
          this.$api.outfall
            .listByCompanyId({ companyId: this.dataTable.companyId })
            .then((ress) => {
              this.outfalls = ress.data
              let map = new Map()
              for (let i = 0; i < this.dataTable.outfallLedList.length; i++) {
                let outfallName = this.outfallNameByoutfallId(
                  this.dataTable.outfallLedList[i].outfallId
                )
                let pollutantName = this.pollutantNameBypollutantId(
                  this.dataTable.outfallLedList[i].pollutantId
                )
                map.has(outfallName)
                  ? map.set(
                      outfallName,
                      map.get(outfallName) + '，' + pollutantName
                    )
                  : map.set(outfallName, pollutantName)
              }
              for (let entry of map) {
                this.outfallPollutants.push({
                  outfallName: entry[0],
                  pollutantNames: entry[1],
                })
              }
            })
          ...
          if (this.dataTable.heartTime === null) {
            ...
            this.dataTable.heartTime = formatTime(
              new Date(this.dataTable.heartTime)
            )
          }
        } else {
          this.$message({
      ...
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          for (let i = 0; i < this.outfallPollutant.length; i++) {
            for (
              let j = 0;
              j < this.outfallPollutant[i].pollutents.length;
              j++
            ) {
              if (this.outfallPollutant[i].pollutents[j].checked) {
                let outfallLed = {}
                outfallLed.outfallId = this.outfallPollutant[i].outfallId
                outfallLed.pollutantId = this.outfallPollutant[i].pollutents[
                  j
                ].id
                outfallLed.pollutantSort = this.outfallPollutant[i].pollutents[
                  j
                ].showOrder
    ...
    listOutfall: function (companyId) {
      this.$api.outfall
        .listByCompanyId({ companyId: companyId })
        .then((res) => {
          ...
          for (let i = 0; i < this.outfalls.length; i++) {
            ...
            this.$api.pollutant
              .listByOutfallId({ outfallId: element.id })
              .then((res) => {
                ...
                if (this.operation) {
                  ...
                  for (
                    let j = 0;
                    j < this.dataForm.outfallLedList.length;
                    j++
                  ) {
                    if (
                      element.id === this.dataForm.outfallLedList[j].outfallId
                    ) {
                      for (
                        let z = 0;
                        z < this.outfallPollutant[i].pollutents.length;
                        z++
                      ) {
                        if (
                          this.outfallPollutant[i].pollutents[z].id ===
                          this.dataForm.outfallLedList[j].pollutantId
                        )
